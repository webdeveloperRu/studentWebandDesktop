...
class AuthService {
  ...
  logout() {    
    return axios.get(API_URL + 'logout',{headers:  {'apikey': store.state.auth.user.token, "accept": "application/json"}}).then(response => {
  ...
  register(user) {
    return axios.post(API_URL + 'register', {
      ...
      "password": user.password,
      "otp": user.otp
    },{headers:  {'Auth': 'allow'}}).then(response => {
      return response;
    })
    .catch(err => {    
      return err;
  ...
  changePassword(password, newpassword, verifypassword) {
    ...
    }, {headers:  {'apikey': store.state.auth.user.token}}).then(response => {
  ...
  userLog() {    
    return axios.get(API_URL + 'userlog/', {headers:  {'apikey': store.state.auth.user.token}}).then(response => {
      return response;      
    })
    .catch(err => { 
      return err;
  ...
  getSettings() {
    return axios.get(API_URL + 'settings', { headers: authHeader() }).then(response => {
      return response;
    })
    .catch(err => { 
      return err;
  ...
  removeAvatar() {
    return axios.delete(API_URL + 'settings/avatar', 
    {headers:  {'apikey': store.state.auth.user.token, "accept": "application/json"}}).then(response => {
      return response;
    })
    .catch(err => {        
      return err;
