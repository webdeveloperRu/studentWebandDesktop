...
? { status: { loggedIn: true}, user, email_code:'', email_resetpassword:'', user_log:'',twofa_logged: false, login_userinfo: null, rememberme: false, user_settings: [] } 
: { status: {loggedIn: false}, user: null, email_code:'', email_resetpassword:'', user_log:'', twofa_logged: false, login_userinfo: null, rememberme: false, user_settings: []};
...
export const auth = {
  ...
  actions: {
    ...
    login({ commit }, [user, rememberme]) {      
       return  AuthService.login(user, rememberme).then(        
          res => {                    
              ...
              else if( res.response.status == 401 && res.response.data.message == 'otp code is requested'){
                commit('setTwoFactorLogIn', {user, rememberme})
    ...
    logout({commit}) {
      return  AuthService.logout().then(
        res => {
          if (res.status == 200) {
            commit('twofaManage/RESET_MODULE', null, {root: true})
            commit('productManage/RESET_MODULE', null, { root: true })
            commit('categoryManage/RESET_MODULE', null, { root: true })
            commit('lessonManage/RESET_MODULE', null, { root: true })
            commit('commentManage/RESET_MODULE', null, { root: true })
            commit('RESET_MODULE', null, { root: true })
            commit('RESET_MODULE')
    ...
    changePassword({ commit }, [password, newpassword, verifypassword]) {
      return AuthService.changePassword(password, newpassword, verifypassword).then(
        res => {
          if(res.success == true) {
  ...
  mutations: {
    ...
    updateSettingsSuccess() {
      store.state.notification_icon = 'info';
      store.state.notification_color = 'primary';
      store.state.notification_text = 'User profile settings are successfully updated.'
    },
